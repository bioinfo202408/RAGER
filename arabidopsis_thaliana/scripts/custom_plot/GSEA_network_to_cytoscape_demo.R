...
enrich_result <- read.csv(file = enrich_file, header = TRUE)
...
expData <- read.csv(expr_file, row.names = 1)
...
symbolIDs <- transId(
   id = entrez_ids,
   transTo = "symbol", org = "arabidopsis", keepNA = FALSE
)
...
write_graph(enrich.net.graph, output_file, format = "graphml")
