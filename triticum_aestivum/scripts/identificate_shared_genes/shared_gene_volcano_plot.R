...
g_symbol <- mapIds(org.At.tair.db,
                   keys = geneID,
                   column = "SYMBOL",
                   keytype = "TAIR",
                   multiVals = "first")
...
expData_filtered <- expData %>%
   filter(abs(log2FoldChange) < 10 & -log10(padj) < 100)
...
ggplot() +
   ...
   scale_y_continuous(expand = expansion(add = c(2, 0)), limits = c(0, 110), breaks = seq(0, 80, by = 20)) +
