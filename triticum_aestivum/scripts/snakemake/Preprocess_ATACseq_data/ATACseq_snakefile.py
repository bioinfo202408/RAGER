...
rule all:
    input:
        ...
        f"{GENEANNO_DIR}/gencode.vM25.annotation.bed",
...
rule generate_qc_plots:
    ...
    shell:
        f"""
        Rscript --vanilla {SCRIPTS_DIR}/quality_control_and_mapping/ATACseqQC.R {input.bam} {output.fragment_size} {ATACseq_BOWTIE2_DIR}/{{sample}}/splited {output.PT} {output.NFRscore} {output.TSSEscore} {output.cumulative_percentage} {output.heatmap} {output.coverage_curve} {input.txdb}
        """
...
rule annoATACPeaks:
    ...
    shell:
        f"""
        Rscript --vanilla {SCRIPTS_DIR}/quality_control_and_mapping/annoATACPeaks.R {input.up_peak} {input.down_peak} {output.anno_up} {output.anno_down} {input.txdb}
        """
