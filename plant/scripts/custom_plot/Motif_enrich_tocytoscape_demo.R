...
# Arabidopsis: input gene IDs are expected to be TAIR/AGI locus IDs like AT1G01720
geneID <- filtered_top10_network$V3 %>%
  as.character() %>%
  na.omit()

# TAIR -> SYMBOL mapping (fallback to original TAIR ID if SYMBOL is missing)
g_symbol <- AnnotationDbi::mapIds(org.At.tair.db,
                                  keys = geneID,
                                  keytype = "TAIR",
                                  column = "SYMBOL",
                                  multiVals = "first") %>%
