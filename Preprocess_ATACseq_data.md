...
vim ./arabidopsis_thaliana/scripts/snakemake/Preprocess_ATACseq_data/config.yaml
#triticum aestivum
vim ./triticum_aestivum/scripts/snakemake/Preprocess_ATACseq_data/config.yaml  
...
#arabidopsis thaliana
references:
  gtf_file: "reference/geneanno/Arabidopsis_thaliana.TAIR10.62.gtf"   # Gene annotation file
  bed_file: "reference/geneanno/Arabidopsis_thaliana.TAIR10.62.bed"   # Gene annotation in BED format
  bowtie2_index: "Arabidopsis_thaliana_TAIR10"                        # Bowtie2 index name
#triticum aestivum
references:
  gtf_file: "reference/geneanno/Triticum_aestivum.IWGSC.62.gtf"   # Gene annotation file
  bed_file: "reference/geneanno/Triticum_aestivum.IWGSC.62.bed"   # Gene annotation in BED format
  bowtie2_index: "Triticum_aestivum_IWGSC"                        # Bowtie2 index name
```
...
#arabidopsis thaliana
  bowtie2:                        # Alignment parameters
    threads: 5                    # Number of CPU threads
    index_name: "Arabidopsis_thaliana_TAIR10"         # Reference genome index name
#triticum aestivum
  bowtie2:                        # Alignment parameters
    threads: 5                    # Number of CPU threads
    index_name: "Triticum_aestivum_IWGSC"         # Reference genome index name
```
...
```yaml
  macs2:                          # Peak calling parameters
    genome_size: "mm"            # Genome size ("mm" for mouse, "hs" for human,"at" for Arabidopsis thaliana)
    qvalue: 0.05                 # Q-value threshold
    keep_dup: "all"              # Duplicate read handling
```

**How to modify**: 
- Change `genome_size` to `"hs"` for human samples, `"mm"` for mouse, `"at"` for Arabidopsis thaliana
...
#arabidopsis thaliana
cd ~/PROJECT/RAGER/arabidopsis_thaliana  #Change the directory to PROJECT/RAGER/arabidopsis_thaliana as the current directory
snakemake --snakefile ./scripts/snakemake/Preprocess_ATACseq_data/ATACseq_snakefile.py --configfile ./scripts/snakemake/Preprocess_ATACseq_data/config.yaml -j 10

#triticum aestivum
cd ~/PROJECT/RAGER/triticum_aestivum  #Change the directory to PROJECT/RAGER/triticum_aestivum as the current directory
snakemake --snakefile ./scripts/snakemake/Preprocess_ATACseq_data/ATACseq_snakefile.py --configfile ./scripts/snakemake/Preprocess_ATACseq_data/config.yaml -j 10
```
## **List of processes**
