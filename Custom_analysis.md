...
#plant
vim ./plant/scripts/snakemake/Custom_genes_analysis/config.yaml 
...
- Update `tf_mapping_file` for different species (e.g.`datasets/Homo_sapiens_TF.txt` for human , `datasets/Ath_TF_list.txt` for plant)
...
#plant
mkdir -p ./plant/datasets/Custom_genes_analysis
vim ./plant/datasets/Custom_genes_analysis/custom_genes.txt
```
